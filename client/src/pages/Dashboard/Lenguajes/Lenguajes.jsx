...
export const FlagCard = ({ flagImg, buttonImg1, buttonImg2 }) => {
  ...
  return (
    <>
      <div className="bg-white rounded shadow-md p-10 flex flex-col items-start h-4/5 w-4/5 gap-8 mt-4">
        <div className="flex justify-between items-start w-full">
          <div className="flex flex-row items-center gap-6 px-16">
            <img
              src={flagImg}
              alt="Bandera"
...
export const Lenguajes = () => {
  ...
  const onSubmitLanguage = async (data) => {
    if (selectedImage === selectPhoto) {
      toast.warning(
        "Debes seleccionar una imagen, clickea el icono azul"
      );
      return;
    }
    ...
    if (!/^[A-Za-z\s]+$/.test(idioma)) {
      toast.warning("El idioma solo puede contener caracteres del alfabeto y espacios");
      return;
    }
